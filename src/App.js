...
import './App.css';
import PersonInfo from './Components/PersonInfo/PersonInfo.js'
...
filterPersonById (cardNum) {
  const { data } = this.state
  const filteredPeopleArray = []
  for ( let i = 0; i < data.length; i++) {
    if (data[i].id === cardNum ) {
      filteredPeopleArray.push(data[i])
      return filteredPeopleArray
    }
  }
}

handlePrevious (cardNum) {
  if (cardNum <= 1) {
...
handleNext (cardNum) {
  ...
  render () {
    const { data, cardNum } = this.state
    let fitleredPerson = data.length ? data.filter(person => cardNum === person.id).map(person => {
      return <PersonInfo id={person.id} name={person.name} city={person.city} country={person.country} employer={person.employer} title={person.title} favoriteMovies={person.favoriteMovies}/>
    }) : null
    return (
      <div className="app">
        <header>
          ...
          {fitleredPerson}
            <div className="navBarContainer">
                <button onClick={() => this.handlePrevious(cardNum)}> {`< Previous`} </button>
                <div className="centerButtonsContainer">
                    <button> Edit </button>
                    <button> Delete </button>
                    <button> New </button>
                </div>
                <button  onClick={() => this.handleNext(cardNum)}> {`Next >`} </button>
            </div>
          </section>
      </div>
    )
